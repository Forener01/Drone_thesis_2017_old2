...
Strategy::Strategy()
{
  ...
  // TODO: subscribe to channels in function of the role ...
...
void Strategy::the_8_trajectory()
{
  oldstrategychosen = strategychosen;
  strategychosen = 8.0;
}
...
// This function send the strategy number and the position chosen to the path_planning.
...
int main(int argc, char** argv)
{
  ...
  else if (myStrategy.FindRole() == 5)
  {
    while (ros::ok())
    {
      myStrategy.strategychosen = 8.0;
      myStrategy.Takeoff();
      myStrategy.publish_strategy();
      ros::spinOnce();
      r.sleep();
      ros::Duration(10).sleep();  // Wait for 10s
      myStrategy.the_8_trajectory();
      myStrategy.publish_strategy();
      ros::spinOnce();
      r.sleep();
      myStrategy.Intheair = true;
