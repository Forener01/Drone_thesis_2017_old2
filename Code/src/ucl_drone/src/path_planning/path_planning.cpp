...
int main(int argc, char **argv) {
  ...
  while (ros::ok()) {
    ...
    // }
    //
    // // Strategy = GOTO, the drone goes directly to the position writed in the
    // message from the
    // // strategy.
    // else if (myPath.lastStrategyReceived.type == 3.0)
    // {
    //   myPath.takeoff = true;
    //   myPath.SetRef(myPath.lastStrategyReceived.x,
    //   myPath.lastStrategyReceived.y + 0.5,
    //                 myPath.next_z,
    //                 myPath.next_rotZ);  // Pas correct, ne tient pas compte
    //                 de la
    ...
    //   myPath.takeoff = true;
    //   myPath.SetRef(myPath.lastStrategyReceived.x,
    //   myPath.lastStrategyReceived.y, myPath.next_z,
    //                 myPath.next_rotZ);
    //   myPath.publish_poseref();
    // }
    //
    ...
    //            (myPath.lastPoseReceived.y) * (myPath.lastPoseReceived.y)) <
    //            0.15)
    //   {
    //     myPath.takeoff = false;
    //     myPath.landing = true;
    ...
    // }
    //
    // // Strategy = Seek (explore), the drone initializes the grid. Then this
    // is the
    ...
    //
    //   myPath.advanced_xy_desired(myPath.lastPoseReceived.x,
    //   myPath.lastPoseReceived.y,
    //                              &myPath.poseRefX, &myPath.poseRefY);
    //   myPath.SetRef(myPath.poseRefX, myPath.poseRefY, myPath.next_z,
    //   myPath.next_rotZ);
...
// Function that gives a yaw angle in order to always face to the direction to
// go. We don't use it
// at this moment.
